...
The average per-layer time budget at the final 2-bit + OS page cache configuration (2.9\,ms/layer $\times$ 60 = 174\,ms/token $\approx$ 5.7\,tok/s) breaks down as follows: \textbf{Expert I/O} via parallel \texttt{pread()}: 1.37\,ms (47\%); \textbf{CMD1 wait} (GPU projections + deferred CMD3): 0.87\,ms (30\%); \textbf{CMD2 wait} (GPU routing + shared expert): 0.45\,ms (15\%); \textbf{CPU attention} (BLAS-accelerated GatedDeltaNet): 0.15\,ms (5\%); other overhead: 0.06\,ms (2\%).
Expert I/O dominates, consuming nearly half of the per-layer budget.
