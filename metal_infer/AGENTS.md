...
- `python3 benchmark_profiles.py --model <snapshot> --weights model_weights.bin --manifest model_weights.json --vocab vocab.bin --prompt-tokens /tmp/prompt_tokens.bin --emit-script /tmp/run_benchmarks.zsh` emits a shell-driven sweep when Python child launches are unreliable on Metal.
